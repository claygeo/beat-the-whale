...
1. Show injected events as ⚡/🚀 chart markers (so the player sees where their event hit).
2. Arcade "best run" local high-score (localStorage) — replay-value hook.
3. Mid-run speed change (smooth accumulator clock) / live-updating forming candle for true high-TPS shimmer.
4. Final full `/qa` functional pass on both modes; then `/review` + codex pre-landing.
