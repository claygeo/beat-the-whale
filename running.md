...
**NEXT — stretch polish (pick highest-value):**
1. **Arcade "best run" high-score** (localStorage) — "beat your best $X" on the result + start screen; the come-back hook.
2. Final full `/qa` functional pass on BOTH modes; then `/review` + codex pre-landing review.
3. Mid-run speed change (smooth accumulator clock) / live-updating forming candle for true high-TPS shimmer.
4. Minor: arcade event markers use amber (opponent color) — consider a neutral/blue marker color in arcade so they don't read as "Market" trades (emoji already disambiguates).
