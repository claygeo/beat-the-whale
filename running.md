...
**NEXT — QA + polish + balance:**
1. **Bot difficulty / fairness** (threshold → /codex): in testing the momentum bot (look=4, $3k×3) caught both crash & pump and led — verify it's beatable with good play (~40-55% win), tune look-back / size / add a small handicap or latency if it feels unbeatable. FUN > hard.
2. **`/qa-design-review` BOTH modes** on mobile 375 + X-webview 390×550 vs DESIGN.md; tighten the arcade selector row (4 events + 3 speeds scroll off-screen → make speed discoverable / regroup), touch targets, spacing.
3. Stretch: mid-run speed change (smooth, accumulator clock); live-updating forming candle for true high-TPS shimmer; arcade "best run" local high-score.
