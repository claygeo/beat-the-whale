...
✅ **QA-DESIGN-REVIEW pass 1 DONE (verified mobile 375 + X-webview 390×550, 0 console errors):** (a) **DESIGN.md updated** to record the operator-approved Inter-for-text / mono-for-numbers override + the two-mode product (was still spec'ing Jakarta/Geist → future QA would mis-flag). (b) **arcade selector row fixed** — was a horizontal scroll of 4 events + 3 speeds with speed off-screen; now the 4 events are an even **4-col grid** (all visible, no scroll, stacked emoji+label, ~52px touch targets) and **Speed moved to the footer above Play** (pre-run, discoverable). Confirmed both modes fit + play in the constrained webview; race is genuinely close (balance holding).
✅ **Event markers DONE (`407c38a`, live):** injected events drop a ⚡/🩸 (down) or 🚀/📈 (up) marker at their trigger candle — the player sees where their manipulation hit the tape. Reuses the marker system; verified live.
✅ **Arcade high-score + marker color DONE (live, verified mobile 375, 0 console errors):** localStorage `btw_arcade_best` tracks your best run PnL; shown as "Best run · ±$X" on the arcade start screen and a gold "🏆 New best run · +$X" badge on the result (the come-back hook — persists across sessions). Arcade event markers recolored **player-blue** (#4a9eff) vs whale-amber, so they read as YOUR events not the Market's (added optional `color` to WhaleMarker). Verified: lost to a strong-drift Market +$464 but still logged a new best +$233 → reads fine (round result + lifetime best are distinct).
✅ **Codex pre-landing review DONE (`9a3cebd`):** reviewed scenario.ts — NO blocking issues (determinism/purity/index-isolation all hold). 2 defensive SHOULD-FIX applied + tested: recompute vol/return caps + finite price guard (button-mash can't overflow), non-finite input clamps on all public fns + 20-event cap. 27 tests green.
...
**NEXT — final ship gate:**
1. **Final functional smoke test** — esp. the whale **Daily → ranked submit → leaderboard** path (refactored heavily during the mode-toggle work; re-verify it still loads + submits) + featured/sample/wallet + arcade events/speed/high-score. (preview renderer flaky during active rounds → screenshot static states / restart preview on timeout.)
2. Launch asset: 10s screen-rec GIF (Arcade crash→pump→win) + X post copy.
