...
**NEXT — wire Scenario/Arcade into the UI (codex steps 1/4/5/6/8):**
1. App-level **`mode:'replay'|'scenario'`** + a top toggle (tabs "🐋 Whale" / "🎮 Arcade"); keep whale-replay path 100% untouched.
2. **Seed** picked at each run-start (`Date.now()` is fine — only scenario.ts internals must stay pure). New-run = new seed.
3. Drive **CandleChart** from `pathToCandles(path, candleMs≈1000, uptoMs=elapsed)` revealed progressively; drive the paper **sim** from those synthetic candles (reuse simulate()); reuse RaceLane + EquityChart with the **ghost bot** (`botCurve`) as the opponent (recompute botOrders when events change).
4. **Scenario buttons** (4, big + thumb-friendly) → `injectEvent(path,key,elapsed)` → recompute path + bot. **Speed mode** = clock multiplier (1×/2×/4×). Stretch = update the forming candle live (true high-TPS feel).
5. Then `/qa-design-review` (mobile 375 + X-webview 390×550) + deploy.
Deferred: share card, in-DB PnL recompute, pg_cron daily freeze. Minor: `/qa-design-review` polish nits on whale mode (already verified across 3 viewports).
